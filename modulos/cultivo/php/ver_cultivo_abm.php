...
//VARIEDAD	
	...
	//PARCELAS
	$sql="Select * from  tabla_65_tbl_parcela  order by id_tabla65 ASC";
	$rs = $pdo->query($sql);//
	$num_rows = $rs->rowCount();
	if ($num_rows>0)
	{
		while ($row_cultivo = $rs->fetch())  
		{
				...
				if ($id_tabla65==$rela_tabla65) 
				{
						...
						$t->set_var("NOMBRE",($row_cultivo["id_tabla65"]));	// aca esta el error

				}else{
						$t->set_var("ID",$row_cultivo["id_tabla65"]);
						$t->set_var("NOMBRE",($row_cultivo["id_tabla65"]));	
				}
