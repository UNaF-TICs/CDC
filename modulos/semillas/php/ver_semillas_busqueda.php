...
//Archivos comunes
$t->set_file(array(
	"ver"				=> "ver_semillas_busqueda.html",
	"un"				=> "un_semillas.html",
...
$titulo="Listado de semilla";
$t->set_var("titulo",$titulo);

//
//Otras Funciones
//$t->set_var("funcion_excel","modulos/semillas/php/exportar_excel.php?tipo=xls");
//$t->set_var("funcion_doc","modulos/semillas/php/exportar_excel.php?tipo=doc");
//$t->set_var("funcion_pdf","modulos/semillas/php/exportar_excel.php");
//

$url="'modulos/semillas/php/ver_semillas.php'";
$id="'listado_semilla'";
$vars="'es_buscar=si&id_tablamodulo=$id_tablamodulo&";	
$vars.="tabla25_nombre='+ver_busqueda_semilla.tabla25_nombre.value";	
...
if ($num_rows>0)
{
	while ($row = $rs->fetch())
	{
		...
		$vars="'nombre_funcion=borrar_semilla&";
...
else
{
	...
	if($offset>1)
	{
		$pag.="<td><a href=\"javascript:cargar_post('modulos/semillas/php/ver_semillas.php','listado_semilla','offset=$off&id_tablamodulo=$id_tablamodulo');\"><< Anterior</a> | </td>";
	}
	else
	{
		$pag.="<td></td>";
	...
	for($i=$com;$i<=$ter;$i++)
	{
		...
		{
			$pag.="<a href=\"javascript:cargar_post('modulos/semillas/php/ver_semillas.php','listado_semilla','offset=$i&id_tablamodulo=$id_tablamodulo');\">$i</a>&nbsp;";
		}				 	 
	}
	$pag.="</td>";
					// Siguiente >>
	...
	{
		$ofs=$offset+1;
		$pag.="<td> | <a href=\"javascript:cargar_post('modulos/semillas/php/ver_semillas.php','listado_semilla','offset=$ofs&id_tablamodulo=$id_tablamodulo');\">Siguiente >></a></td>";
	}else{
		$pag.="<td></td>";
	}
